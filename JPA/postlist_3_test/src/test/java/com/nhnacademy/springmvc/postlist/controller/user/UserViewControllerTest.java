...
class UserViewControllerTest {
    ...
    void views() throws Exception {
        ...
        mockMvc.perform(get("/user_view?id=304").session(session))
                .andDo(print())
    ...
    void viewsAdmin() throws Exception {
        ...
        mockMvc.perform(get("/user_view?id=2").session(session))
                .andDo(print())
                .andExpect(status().isOk())
                .andExpect(view().name("user/userview"));
    }
    ...
    void viewsIdError() throws Exception {
        ...
        User user = new User();
        session.setAttribute("user",user);
        mockMvc.perform(get("/user_view?id=101211").session(session))
                .andDo(print())
