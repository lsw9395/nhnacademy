...
(function(){
    ...
    async function createTodoList(){
        ...
        const datainfo = navi.getYear() + "-" + navi.getMonth();
        const todoItems = await store.getTodoItemList(datainfo);
        ...
        for(let i=1; i<=daysInMonth; i++ ){
           //https://developer.mozilla.org/ko/docs/Web/API/Document/importNode
            ...
            displayTodoItemList(zeroDate,todoItems);
    ...
    async function todoSubmit(event){
        ...
        if(validateForm(event.target)){
            ...
            try{
                ...
                const datainfo = navi.getYear() + "-" + navi.getMonth();
                const todoItems = await store.getTodoItemList(datainfo);
                displayTodoItemList(todoDate,todoItems);
            }
        }
    }
    ...
    async function  displayTodoItemList(todoDate,todoItems){
        clearTodoItemList(todoDate);
        try{
            for(let i=0; i<todoItems.length;i++){
                if(todoItems[i].todoDate===todoDate){
                    const todoItemList = document.getElementById("todo-item-list-" + todoDate);
                    console.log(todoItems[i].todoSubject);
